...
(function() {
  ...
  window.fetch = function() {
    ...
    return origFetch.apply(this, args).then(function(resp) {
      try {
        var ct = '';
        if (resp.headers && resp.headers.get) ct = resp.headers.get('content-type') || '';
        if (ct.indexOf('event-stream') !== -1 && resp.body && resp.body.getReader) {
          var clone = resp.clone();
          var reader = clone.body.getReader();
          var tdec = new TextDecoder();
          var buf = '';
          var total = 0;
          (function pump() {
            reader.read().then(function(r) {
              if (r.done) {
                if (buf) send({ type: 'sse', url: url, data: buf.slice(0, 800) });
                return;
              }
              buf += tdec.decode(r.value, { stream: true });
              total += r.value.length;
              if (buf.length >= 800) {
                send({ type: 'sse', url: url, data: buf.slice(0, 800) });
                buf = buf.slice(800);
              }
              if (total < 5000000) pump();
            }).catch(function(){});
          })();
        } else if (ct.indexOf('json') !== -1 || ct.indexOf('text') !== -1) {
          var c2 = resp.clone();
          c2.text().then(function(t) {
            send({ type: 'fetch_res', url: url, status: resp.status, data: String(t).slice(0, 2000) });
          }).catch(function(){});
        }
      } catch(e) {}
      return resp;
  ...
  XMLHttpRequest.prototype.open = function(method, url) {
    ...
    this.__re_url = String(url);
    return origXHROpen.apply(this, arguments);
  };
  var origXHRSend = XMLHttpRequest.prototype.send;
  XMLHttpRequest.prototype.send = function() {
    var xhr = this;
    send({ type: 'xhr', method: xhr.__re_method, url: xhr.__re_url });
    xhr.addEventListener('load', function() {
      try {
        var ct = xhr.getResponseHeader('content-type') || '';
        if (ct.indexOf('json') !== -1 || ct.indexOf('text') !== -1) {
          send({ type: 'xhr_res', url: xhr.__re_url, status: xhr.status, data: String(xhr.responseText || '').slice(0, 2000) });
        }
      } catch(e) {}
    });
  ...
  if (OrigWS) {
    var WSProxy = function(url, protocols) {
      ...
      var us = String(url);
      send({ type: 'ws_open', url: us });
      var origSend = ws.send;
      ws.send = function(d) {
        try { send({ type: 'ws_send', url: us, data: String(d).slice(0, 800) }); } catch(e) {}
        return origSend.apply(ws, arguments);
      };
      ws.addEventListener('message', function(ev) {
        try { send({ type: 'ws_msg', url: us, data: String(ev.data).slice(0, 800) }); } catch(e) {}
      });
      return ws;
    };
  ...
  console.log('%c[RE] recorder active %c(v4)', 'color:#0f0;font-weight:bold', 'color:#666');
  return 'injected_v4';
})();
`;
...
browser.runtime.onMessage.addListener(function(msg, sender) {
  ...
  if (msg.action === 'pushAction') {
    if (state.recording) {
      ...
      if (state.actions.length > 5000) state.actions.splice(0, state.actions.length - 5000);
    }
